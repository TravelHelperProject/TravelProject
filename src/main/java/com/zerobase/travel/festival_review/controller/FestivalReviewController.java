...
public class FestivalReviewController {
    ...
    public ResponseEntity<String> reviewInsert(@RequestBody FestivalReviewAddDTO reviewDTO,
    ...
    public ResponseEntity<String> reviewModify(@RequestBody FestivalReviewModifyDTO festivalReviewModifyDTO,
                                               @PathVariable long festivalId,
                                               @PathVariable long reviewId,
                                               @AuthenticationPrincipal UserDTO user) {
        if(service.get(reviewId).getUserId() == user.getUser_id()) {

            FestivalReviewDTO festivalReviewDTO = FestivalReviewDTO.builder()
                    .content(festivalReviewModifyDTO.getContent())
                    .grade(festivalReviewModifyDTO.getGrade())
                    .build();
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body("아이디가 일치하지 않습니다.");
    }
